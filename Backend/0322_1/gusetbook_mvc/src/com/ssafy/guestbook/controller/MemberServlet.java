...
public class MemberServlet extends HttpServlet {
	...
	private String loginMember(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if(memberDto != null) {//로그인 성공
				...
				if("saveok".equals(idsv)) {//아이디 저장 체크했다면
					//쿠키 세팅
					Cookie cookie = new Cookie("loginid",id);
					cookie.setMaxAge(60*60*24*365*20);//초단위
					cookie.setPath(request.getContextPath());//...여기까진 메모리에만 쿠키만든거
					
					response.addCookie(cookie);//응답에 쿠키담기
					
				}else{//아이디 저장체크 x
					Cookie[] cookies = request.getCookies(); //브라우저에서 서버로 자동으로 넘긴 쿠키들
					if(cookies != null){
						for(int i=0; i<cookies.length; i++){
							if(cookies[i].getName().equals("loginid")){
								cookies[i].setMaxAge(0); //지우는건 따로 없어서 0으로 함
								response.addCookie(cookies[i]);// 바뀐 쿠키 알려줘야지
								break;
							}
						}
					}
	...
	private String loginOutMember(HttpServletRequest request, HttpServletResponse response) {
		HttpSession session = request.getSession();
//		session.setAttribute("userifo", null);
		session.removeAttribute("userinfo");
		session.invalidate();//연결된 정보 다없애기
		return "/index.jsp";
	}
