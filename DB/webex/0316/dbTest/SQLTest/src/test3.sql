...
--     ------------
--            6,461
select format(avg(salary),1) as "사원급여평균"
from employees;

select round(avg(salary),1) as "사원급여평균"
from employees;
...
--이런 질문은 만들 수 없음. 사원이름은 100개 넘는데, 부서별 급여평균은 12개빡에 안됨. 이런거 보여줄 수 없어
...
--아래는 비효율적인 방법임.만약 부서가 10000개라면, group으로 10000개로 나누고, 여기서 꼴랑 두개만 뽑아오는 거잖아.
select department_id, max(salary) as "max_salary"
from employees
group by department_id
having department_id in(10,20)
order by department_id;
--효율적인 방법: 먼저 두개만 딱 뽑아온 담에 그룹으로 나누는게 좋음
select department_id, max(salary) as "max_salary"
from employees
