...
-- 회사에 존재하는 모든 부서....이건 사원이 존재하지 않는 부서도 다 나옴
select department_id
from departments;
...
select employee_id 사번, first_name "이름", salary as 급여, salary *12 연봉
from employees;


-- 모든 사원의 사번, 이름, 급여, 급여 * 12 (연봉), 커미션, 커미션포함 연봉 검색.
select employee_id 사번, first_name "이름", salary as 급여, salary *12 연봉, commission_pct, salary*(1+ifnull(commission_pct,0))*12 "커미션포함연봉"
from employees;
...
-- 이름에 'x'가 들어간 사원의 사번, 이름..._(언더바)이름
select employee_id , first_name, salary, department_id 
from employees
