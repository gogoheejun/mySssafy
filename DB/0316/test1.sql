...
values (idx,'heejun', '양희준','1234','jun','naver.com',now());
...
UPDATE ssafy_member
SET userpwd = 1111, emaildomain = 'korea.co.kr'
WHERE userid = 'heejun';
...
select employee_id as "사원번호", last_name as "이   름", salary as "급여"
from employees;
...
select employee_id as "사원번호", concat(first_name,' ', last_name) as "이 름", concat(salary, '달러') as "연봉"
from employees;
...
--Mysql은 대소문자 구분 안함. binary쓰면 대소문자 구분함
select * 
from employees
where last_name=binary('KIng');          --->문자열검색할때 대.소문자를 구분
...
from employees
where binary(job_id) like '%CL%';


-- ex15)업무ID가  IT로 시작하는 사원들의 이름,업무ID,부서ID를 출력하시오  -- 5건
select last_name, job_id, department_id
from employees
where binary(job_id) like 'IT%';
